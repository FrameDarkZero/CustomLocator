...
class CustomLocator(object):
    ...
    def __init__(self, name, prefix=None, chain_name=None, orientation="x"):
        self.name = name
        self.prefix = prefix
        ...
        if name == None:
            ...
            self.locator = self.create_locator(name, prefix, chain_name, orientation)
    ...
    def get_or_create_shader(self, shader_name, shape_node, hsv):
        ...
        if shader_name in self.shader_cache:
            ...
            try:
                ...
                # If shader doesn't exist or has no connections, fall through
                pass
            
            # If we get here, the cache entry is stale
    ...
    def create_locator(self, name, prefix, chain_name, orientation):
        ...
        orange = self.get_or_create_shader("orange_loc_lambert", sphere, hsv=(21.246, 1, 1))
        red = self.get_or_create_shader("red_loc_lambert", x_cone, hsv=(3.900, 1, 1))
        green = self.get_or_create_shader("green_loc_lambert", y_cone, hsv=(103.302, 1, 1))
        blue = self.get_or_create_shader("blue_loc_lambert", z_cone, hsv=(217.074, 1, 1))
        
        # Assign the same shaders to torus shapes
        red_sg = self.shader_cache.get("red_loc_lambert", "red_lambertSG")
        green_sg = self.shader_cache.get("green_loc_lambert", "green_lambertSG")
        blue_sg = self.shader_cache.get("blue_loc_lambert", "blue_lambertSG")
        ...
        if prefix and chain_name:
            locator = cmds.rename(sphere, f"{prefix}_{chain_name}_{name}_loc")
        elif chain_name:
            locator = cmds.rename(sphere, f"{chain_name}_{name}_loc")
        elif prefix:
            locator = cmds.rename(sphere, f"{prefix}_{name}_loc")
        else:
